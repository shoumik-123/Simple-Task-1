...
module.exports.Create = async function (req, res) {
    try {
        let reqBody = req.body;
        let userExist = await Model.findOne(reqBody);
...
module.exports.Update = async function (req, res) {
    try {
        ...
        if (data) {
            return res.status(200).json({ status: "Updated", data: data });
        }
...
module.exports.Delete = async function (req, res) {
    try {
        ...
        if (data.deletedCount === 1) {
            return res.status(204).json({ status: "Success", message: "Record deleted successfully." });
        } else {
